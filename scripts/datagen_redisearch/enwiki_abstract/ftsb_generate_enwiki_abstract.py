...
from common import download_url, generate_setup_json, compress_files, generate_inputs_dict_item, humanized_bytes, \
    ...
    add_deployment_requirements_utilities, init_deployment_requirement, remove_file_if_exists, decompress_file
...
def generate_benchmark_commands(total_benchmark_commands,bench_fname,all_fname,indexname,docs,stop_words):
    ...
    while generated_commands < total_benchmark_commands:
        ...
        if choice == "simple-1word-query" and len(words)>=1:
            generated_row = generate_ft_search_row(indexname, "simple-1word-query", words[0])
            ...
            generated_row = generate_ft_search_row(indexname, "2word-intersection-query", "{}|{}".format(words[0],words[1]))
...
if (__name__ == "__main__"):
    ...
    parser.add_argument('--test-name', type=str, default="enwiki_abstract-hashes", help='the name of the test')
    ...
    add_key_metric(key_metrics, "setup", "throughput", "OverallRates.overallOpsRate", "Overall writes query rate",
                   "docs/sec", "numeric", "higher-better", 1)
    add_key_metric(key_metrics, "setup", "latency", "OverallQuantiles.allCommands.q50",
